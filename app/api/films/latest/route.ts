...
export async function GET() {
    try {
        ...
        const [
            ...
            Film.find({}).sort({ modified: -1 }).limit(5).lean(),

            // 2. 6 phim có view cao (Trending)
            Film.find({}).sort({ views: -1 }).limit(6).lean(),

            // 3. 6 phim Trung Quốc mới
            Film.find({ "countries.name": "Trung Quốc" }).sort({ modified: -1 }).limit(6).lean(),

            // 4. 6 phim Hàn Quốc mới
            Film.find({ "countries.name": "Hàn Quốc" }).sort({ modified: -1 }).limit(6).lean(),

            // 5. 6 phim Âu Mỹ mới
            Film.find({ "countries.name": { $in: ["Âu Mỹ", "Mỹ", "Anh", "Pháp", "Canada", "Đức", "Tây Ban Nha"] } }).sort({ modified: -1 }).limit(6).lean(),

            // 6. 12 phim bộ mới (cho Hero UI)
            Film.find({ "formats.name": "Phim bộ" }).sort({ modified: -1 }).limit(6).lean(),

            // 7. 15 phim lẻ mới
            Film.find({ "formats.name": "Phim lẻ" }).sort({ modified: -1 }).limit(15).lean(),

            // 8. 10 phim hoạt hình mới (cho Split UI)
            Film.find({ "genres.name": "Hoạt Hình" }).sort({ modified: -1 }).limit(10).lean(),
        ]);

        // Helper: Gọi API ngoài để lấy current_episode cho 1 phim
        const fetchCurrentEpisode = async (slug: string): Promise<string | null> => {
            try {
        ...
        return NextResponse.json(
            { message: 'Error fetching latest films', error: (error as Error).message },
            { status: 500 }
        );
    }
}
