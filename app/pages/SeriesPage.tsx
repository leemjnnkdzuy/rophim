...
import {fetchSeriesPageData, type MovieItem} from "@/app/services";
...
function NewestSpotlight({movies}: {movies: Movie[]}) {
	...
	return (
		<section className='w-full px-4 lg:px-32 py-10'>
			<SectionTitle title='Phim Bộ Mới Nhất' />
...
function RecentlyUploadedSection({movies}: {movies: Movie[]}) {
	...
	return (
		<section className='w-full px-4 lg:px-32 py-10'>
			<SectionTitle title='Phim Bộ Mới Cập Nhật' />
			<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 lg:gap-5'>
				{movies.map((movie) => (
					<MovieCard key={movie.id} movie={movie} preferBackdrop />
				))}
...
export default function SeriesPage() {
	...
	const fetchInitialData = useCallback(async () => {
		try {
			setIsLoading(true);
			const data = await fetchSeriesPageData(currentPage, 24);

			setTrendingSeries(
				(data.trendingSeries || []).map((film) =>
	...
	const fetchPaginatedData = useCallback(async (page: number) => {
		try {
			...
			const data = await fetchSeriesPageData(page, 24);
			...
			const section = document.getElementById("all-series-section");
			if (section) {
	...
	return (
		<>
				...
				<div className='relative w-full px-4 lg:px-32 pt-10 pb-6'>
					<div className='flex items-center gap-4 mb-2'>
						<div>
							...
							<p className='text-gray-400 text-sm mt-1'>
								Theo dõi từng tập, nghiện từng giây — phim bộ
								hay đang chờ bạn!
							</p>
						</div>
					</div>
					...
					<div className='flex items-center gap-3 mt-4 flex-wrap'>
						<Badge className='bg-primary/10 text-primary border-primary/20 px-3 py-1.5'>
							...
							{pagination.totalCount.toLocaleString()} phim bộ
						</Badge>
					</div>
				</div>
			</section>
			...
			{/* Section 4: All Series — Paginated Grid */}
			<section
				id='all-series-section'
				className='w-full px-4 lg:px-32 py-10 scroll-mt-20'
			>
				<SectionTitle title='Tất Cả Phim Bộ' />
				...
				:	<div className='flex flex-col items-center justify-center py-20'>
						<div className='p-6 bg-white/5 rounded-2xl border border-white/10 mb-4'>
							<Tv className='h-16 w-16 text-gray-600 mx-auto' />
						</div>
						<h3 className='text-xl font-semibold text-white mb-2'>
							Không có phim bộ
						</h3>
						<p className='text-gray-400 text-center max-w-md'>
							Hiện tại chưa có phim bộ nào trong cơ sở dữ liệu
						</p>
					</div>
				}
			</section>
