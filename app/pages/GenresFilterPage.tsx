...
import {fetchFilterOptions, filterFilms, type MovieItem} from "@/app/services";
...
// ─── Main Page Component ───
interface GenresFilterPageProps {
	filterValue?: string;
}

export default function GenresFilterPage({filterValue}: GenresFilterPageProps) {
	// Decode the value (in case it comes from URL slug)
	const decodedValue = decodeURIComponent(filterValue || "").replace(
		/-/g,
		" ",
	);
	...
	useEffect(() => {
		const loadOptions = async () => {
			try {
				const {formats, genres, countries, years} =
					await fetchFilterOptions();
				setFormatOptions(
					formats.map((f) => (typeof f === "string" ? f : f.name)),
				);
				setGenreOptions(
	...
	const fetchFilms = useCallback(
		async (page: number, isPaging = false) => {
			try {
				...
				// Base genre from URL + any selected extra genres
				const allGenresToFilter = [
					displayValue,
					...selectedGenres,
				].filter(Boolean);
				// Remove duplicates if any (case sensitive?)
				const uniqueGenres = Array.from(new Set(allGenresToFilter));

				const params: Record<string, string | number> = {
					page,
					limit: 24,
					sort: selectedSort,
					genre: uniqueGenres.join(","), // API handles comma-separated values
				};
				...
				const {films, pagination} = await filterFilms(
					params as Parameters<typeof filterFilms>[0],
				);

				const mappedMovies: Movie[] = (films || []).map(
					(film: MovieItem) =>
					...
					<div className='flex items-center gap-4 mb-2'>
						<div>
							<h1 className='text-3xl lg:text-4xl font-black text-white'>
								Thể Loại: {displayValue}
							</h1>
							<p className='text-gray-400 text-sm mt-1'>
								Danh sách phim thuộc thể loại {displayValue}
							</p>
						</div>
					</div>
				...
				<div>
					...
					:	<div className='flex flex-col items-center justify-center py-20'>
							...
							<p className='text-gray-400 text-center max-w-md'>
								Hiện tại chưa có phim nào thuộc thể loại{" "}
								{displayValue}
								{activeFilterCount > 0 &&
